...
class myFoodSuggestion(QWidget, ui_foodsuggestionmenu.Ui_foodSuggestionMenu):
    def __init__(self, mainWindow, currentUserInfo, clfResult_prob, foodWeight):
        ...
        if (len(clfResult_prob)>0):
            ...
            self.lbl_sug1.mousePressEvent(lambda:self.handleAddIntake(mainWindow,clfResult_prob[0][0]))
        if (len(clfResult_prob)>1):
            ...
            self.lbl_sug2.mousePressEvent(lambda:self.handleAddIntake(mainWindow,clfResult_prob[1][0]))
        if (len(clfResult_prob)>2):
            ...
            self.lbl_sug3.mousePressEvent(lambda:self.handleAddIntake(mainWindow,clfResult_prob[2][0]))
        if (len(clfResult_prob)>3):
            ...
            self.lbl_sug4.mousePressEvent(lambda:self.handleAddIntake(mainWindow,clfResult_prob[3][0]))
        if (len(clfResult_prob)>4):
            ...
            self.lbl_sug5.mousePressEvent(lambda:self.handleAddIntake(mainWindow,clfResult_prob[4][0]))
