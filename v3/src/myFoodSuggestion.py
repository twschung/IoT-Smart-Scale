...
import db_access, db_structure, ftp_access, myFoodInformation
currentDir = os.getcwd()

class myFoodSuggestion(QWidget, ui_foodsuggestionmenu.Ui_foodSuggestionMenu):
	def __init__(self, mainWindow, currentUserInfo, clfResult_prob, foodWeight, myFoodInformationSelf):
		...
		if (len(clfResult_prob)>0):
			...
			self.btn_sug1.clicked.connect(lambda: self.handleAddIntake(mainWindow,clfResult_prob[0][0],myFoodInformationSelf))
		if (len(clfResult_prob)>1):
			...
			self.btn_sug2.clicked.connect(lambda: self.handleAddIntake(mainWindow,clfResult_prob[1][0],myFoodInformationSelf))
		if (len(clfResult_prob)>2):
			...
			self.btn_sug3.clicked.connect(lambda: self.handleAddIntake(mainWindow,clfResult_prob[2][0],myFoodInformationSelf))
		if (len(clfResult_prob)>3):
			...
			self.btn_sug4.clicked.connect(lambda: self.handleAddIntake(mainWindow,clfResult_prob[3][0],myFoodInformationSelf))
	...
	def handleAddIntake(self,mainWindow,foodID,myFoodInformationSelf):
		...
		myFoodInformation.myFoodInformation.updateFoodInfo(self,myFoodInformationSelf, foodID, self.currentUserInfo, mainWindow)
		mainWindow.central_widget.removeWidget(mainWindow.central_widget.currentWidget())
