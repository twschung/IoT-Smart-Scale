...
def plot_and_save_weekly_graph(this_week_list, last_week_list, weekday):
    ...
    ax1.set_ylabel('Calorie Comsumption/ kcal')
    ax1.set_xticks(ind + width / 2)
    ax1.set_xticklabels(days)
    ax1.legend((prev_week_series[0], curr_week_series[0]), ('Previous Week','Current Week'))
...
def bar_value(series):
	...
	for x in series:
		height = x.get_height()
		plt.text(x.get_x() + x.get_width()/2., 1.03*height, '%d' %int(height),ha='center', va='bottom')
